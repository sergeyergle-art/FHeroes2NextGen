...
uint32_t Rand::GetIndex( uint32_t size, PCG32 & gen )
{
    return ( size > 1 ) ? uniformIntInInterval( size, gen ) : 0;
}

uint32_t Rand::GetIndex( uint32_t size, uint32_t seed )
{
    if (size > 1) {
        PCG32 gen(seed);
        return uniformIntInInterval(size, gen);
    }
    return 0;
}
...
Rand::Fortuna::Fortuna( int chance_of_luck )
    : _chance_of_luck(chance_of_luck)
    , _current_chance(0)
{
    assert(chance_of_luck >= 0);
}

int Rand::Fortuna::Check( int attempt, PCG32 & gen )
{
    assert(attempt >= 0);

    int chance = _current_chance + _chance_of_luck * attempt;

    if (chance >= 100) {
        _current_chance = chance % 100;
        return chance / 100;
    }

    if (chance > 0) {
        if (uniformIntInInterval(100, gen) < static_cast<uint32_t>(chance)) {
            _current_chance = chance - 100;
            return 1;
        }
    }
