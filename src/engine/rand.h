...
namespace Rand
{
    ...
    // generate random number from [0..size)
    uint32_t GetIndex( uint32_t size, PCG32 & gen );
    uint32_t GetIndex( uint32_t size, uint32_t seed );
    ...
    template <typename Seed, typename Value, std::enable_if_t<std::is_same_v<Seed, uint32_t> || std::is_same_v<Seed, uint64_t>, bool> = true>
    Seed combineSeedWithValue( Seed seed, const Value & v )
    {
        std::hash<Value> hasher;
        return static_cast<Seed>(seed ^ (hasher( v ) + 0x9e3779b9 + ( seed << 6 ) + ( seed >> 2 )));
    }

    class Fortuna
    {
    public:
        Fortuna( int chance_of_luck );

        int Check( int attempt, PCG32 & gen );
