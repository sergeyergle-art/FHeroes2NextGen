...
namespace
{
    inline int32_t AddTwoNumbers( int32_t n1, int32_t n2 )
    {
        auto d = n1 + n2;
        if ( n2 >= 0 ) {
            return (d >= n1) ? d : std::numeric_limits<int32_t>::max();
        }
        else {
            return (d < n1) ? d : std::numeric_limits<int32_t>::min();
        }
    }
}

void Funds::AddWood( int number )
{
    wood = AddTwoNumbers( wood, number );
}
void Funds::AddMercury( int number )
{
    mercury = AddTwoNumbers( mercury, number );
}
void Funds::AddOre( int number )
{
    ore = AddTwoNumbers( ore, number );
}
void Funds::AddSulfur( int number )
{
    sulfur = AddTwoNumbers( sulfur, number );
}
void Funds::AddCrystal( int number )
{
    crystal = AddTwoNumbers( crystal, number );
}
void Funds::AddGems( int number )
{
    gems = AddTwoNumbers( gems, number );
}
void Funds::AddGold( int number )
{
    gold = AddTwoNumbers( gold, number );
}
