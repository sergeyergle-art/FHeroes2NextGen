        ...
        for ( size_t dwellingId = 0; dwellingId < maxNumOfDwellings; ++dwellingId ) {
            if ( _constructedBuildings & castleDwellings[dwellingId] ) {
                if ( le.MouseClickLeft( rectMonster[dwellingId] ) || pressedHotkeyBuildingID == castleDwellings[dwellingId] ) {
                    if ( RecruitMonster( Dialog::RecruitMonster( castleMonster[dwellingId], _dwelling[dwellingId].monsters, true, 2 ) ) ) {
                        // Update available monster count on background.
                ...
                if ( le.isMouseRightButtonPressedInArea( rectMonster[dwellingId] ) ) {
                    Dialog::DwellingInfo( castleMonster[dwellingId], _dwelling[dwellingId].monsters );
...
void Castle::_wellRedrawAvailableMonsters( const uint32_t dwellingType, const bool restoreBackground, fheroes2::Image & background ) const
{
    ...
    case DWELLING_MONSTER1:
        population = _dwelling[0].monsters;
        break;
    case DWELLING_MONSTER2:
        population = _dwelling[1].monsters;
        offset.y += 150;
        break;
    case DWELLING_MONSTER3:
        population = _dwelling[2].monsters;
        offset.y += 300;
        break;
    case DWELLING_MONSTER4:
        offset.x += 314;
        population = _dwelling[3].monsters;
        break;
    case DWELLING_MONSTER5:
        population = _dwelling[4].monsters;
        offset.x += 314;
        offset.y += 150;
        break;
    case DWELLING_MONSTER6:
        population = _dwelling[5].monsters;
        offset.x += 314;
        offset.y += 300;
        break;
    default:
...
void Castle::_wellRedrawBackground( fheroes2::Image & background ) const
{
    ...
    for ( const uint32_t dwellingType : castleDwellings ) {
        ...
        bool isDwellingBuilding = (_constructedBuildings & dwellingType) != 0;

        // Set initial X and Y offset.
        renderPoint.x = offset.x + 269;
        renderPoint.y = offset.y + (isDwellingBuilding ? 18 : 40);

        // Battle level
        std::string textString;
        textString = _( "Level" );
        textString += ": ";
        textString += std::to_string( monster.GetBattleLevel() );
        ...
        textString = _( "Attack" );
        textString += ": ";
        textString += std::to_string( monster.GetMonAttack() );
        ...
        textString += ": ";
        textString += std::to_string( monster.GetMonDefense() );
        ...
        textString = _( "Speed" );
        textString += ": ";
        textString += std::to_string( monster.GetSpeed() );
        ...
        // priority
        textString = _( "Priority" );
        textString += ": ";
        textString += std::to_string( monster.GetPriority() );

        text.set( textString, statsFontType );
        text.draw( renderPoint.x - text.width() / 2, renderPoint.y, background );
        renderPoint.y += text.height( text.width() ) * 2;

        // If dwelling is built show growth and available population.
        if ( isDwellingBuilding ) {
            uint32_t monsterGrown = GetMonstersPerMonth( dwellingType );
            ...
            auto incMonsters = GetPotentialMonstersGrowth( dwellingType );
            if ( incMonsters.second > 0 ) {
                textString = '+';
                textString += std::to_string( incMonsters.first );
                textString += ' ';

                if (incMonsters.second == 1) {
                    textString += _( "tomorrow" );
                }
                else {
                    textString += _( "in" );
                    textString += ' ';
                    textString += std::to_string( incMonsters.second );
                    textString += ' ';
                    textString += _( "days" );
                }
