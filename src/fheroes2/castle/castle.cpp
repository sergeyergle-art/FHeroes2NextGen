...
namespace
{
    ...
    const std::array<uint32_t, 12> allDwellings
    ...
    const std::array<uint32_t, 6> basicDwellings
        = { DWELLING_MONSTER1, DWELLING_MONSTER2, DWELLING_MONSTER3, DWELLING_MONSTER4, DWELLING_MONSTER5, DWELLING_MONSTER6 };
}
...
void Castle::_postLoad()
{
    ...
    if ( _constructedBuildings & DWELLING_MONSTER1 ) {
        _dwelling[0].monsters = Monster( _race, DWELLING_MONSTER1 ).GetGrown();
    }

    if ( _constructedBuildings & DWELLING_UPGRADE2 ) {
        _dwelling[1].monsters = Monster( _race, DWELLING_UPGRADE2 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_MONSTER2 ) {
        _dwelling[1].monsters = Monster( _race, DWELLING_MONSTER2 ).GetGrown();
    }

    if ( _constructedBuildings & DWELLING_UPGRADE3 ) {
        _dwelling[2].monsters = Monster( _race, DWELLING_UPGRADE3 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_MONSTER3 ) {
        _dwelling[2].monsters = Monster( _race, DWELLING_MONSTER3 ).GetGrown();
    }

    if ( _constructedBuildings & DWELLING_UPGRADE4 ) {
        _dwelling[3].monsters = Monster( _race, DWELLING_UPGRADE4 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_MONSTER4 ) {
        _dwelling[3].monsters = Monster( _race, DWELLING_MONSTER4 ).GetGrown();
    }

    if ( _constructedBuildings & DWELLING_UPGRADE5 ) {
        _dwelling[4].monsters = Monster( _race, DWELLING_UPGRADE5 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_MONSTER5 ) {
        _dwelling[4].monsters = Monster( _race, DWELLING_MONSTER5 ).GetGrown();
    }

    if ( _constructedBuildings & DWELLING_UPGRADE7 ) {
        _dwelling[5].monsters = Monster( _race, DWELLING_UPGRADE7 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_UPGRADE6 ) {
        _dwelling[5].monsters = Monster( _race, DWELLING_UPGRADE6 ).GetGrown();
    }
    else if ( _constructedBuildings & DWELLING_MONSTER6 ) {
        _dwelling[5].monsters = Monster( _race, DWELLING_MONSTER6 ).GetGrown();
    }
...
uint32_t Castle::GetMonstersGrowSpeed( uint32_t dwellingId, bool weekBonus ) const
{
    uint32_t actualDwellingId = GetActualDwelling( dwellingId );
    if ( actualDwellingId )
    {
        Monster monster( _race, actualDwellingId );

        uint32_t dwellFactor = 100; /* in percents */

        if ( _constructedBuildings & BUILD_WELL ) {
            dwellFactor += 25;
        }

        if ( _constructedBuildings & BUILD_WEL2 ) {
            dwellFactor += 50;
        }

        if ( _constructedBuildings & BUILD_SPEC ) {
            dwellFactor += 25;
        }

        if ( weekBonus ) {
            auto& week = world.GetWeekInfo();
            if (week.IsMonstersWeek()) {
                if (world.BeginMonth() && (monster.GetID() == week.GetMonster()) ) {
                    dwellFactor *= 2;
                }
            }

            if (week.IsPlagueWeek()) {
                dwellFactor /= 4;
            }
        ...
        return ( monster.GetGrownSpeed() * dwellFactor + 50 ) / 100;
    }

    return 0;
}

uint32_t Castle::GetMonstersPerMonth(uint32_t dwellingId) const
{
    return GetMonstersGrowSpeed(dwellingId, false);
}

std::pair<uint32_t, uint32_t> Castle::GetPotentialMonstersGrowth( uint32_t dwellingId ) const
{
    uint32_t days = 0;
    uint32_t countMonsters = 0;

    auto* dwelling = _getDwelling( dwellingId );
    if ( dwelling != nullptr ) {
        uint32_t growSpeed = GetMonstersGrowSpeed(dwellingId, true);

        uint32_t growth = dwelling->growth;
        if (growth < fheroes2::MONSTER_GROWTH_FACTOR && growSpeed > 0) {
            days = (fheroes2::MONSTER_GROWTH_FACTOR - growth) / growSpeed;
            growth += days * growSpeed;
        }

        // next day
        growth += growSpeed;
        countMonsters = growth / fheroes2::MONSTER_GROWTH_FACTOR;
        if (countMonsters > 0) {
            days += 1;
        }
    }

    return std::make_pair(countMonsters, days);
}
...
Castle::Dwelling * Castle::_getDwelling( const uint32_t buildingType )
{
...
const Castle::Dwelling * Castle::_getDwelling( const uint32_t buildingType ) const
{
    if ( isBuild( buildingType ) )
        switch ( buildingType ) {
        case DWELLING_MONSTER1:
...
void Castle::ActionNewDay()
{
    ...
    if ( world.CountDay() > 1 ) {
        for ( const uint32_t dwellingId : basicDwellings ) {
            ...
            uint32_t growSpeed = GetMonstersGrowSpeed(dwellingId, true);
            uint32_t growth = dwelling->growth + growSpeed;
            uint32_t incMonsters = growth / fheroes2::MONSTER_GROWTH_FACTOR;
            growth -= incMonsters * fheroes2::MONSTER_GROWTH_FACTOR;

            dwelling->monsters += incMonsters;
            dwelling->growth = growth;
        }
    }
}
    ...
    if ( !week.IsPlagueWeek() ) {
        if ( week.IsMonstersWeek() ) {
            for ( const uint32_t dwellingId : allDwellings ) {
                ...
                if ( !mons.isValid() || (mons.GetID() != week.GetMonster()) ) {
                    continue;
                }

                auto* dwellingMonsters = _getDwelling( dwellingId );
                assert( dwellingMonsters != nullptr );

                dwellingMonsters->monsters += GetGrownWeekOf();
                break;
            }
        }
...
bool Castle::RecruitMonster( const Troop & troop, bool showDialog )
{
    ...
    if ( _dwelling[dwellingIndex].monsters < count ) {
        count = _dwelling[dwellingIndex].monsters;
    }
    ...
    _dwelling[dwellingIndex].monsters -= count;
...
uint32_t Castle::getMonstersInDwelling( const uint32_t buildingType ) const
{
    switch ( buildingType ) {
    case DWELLING_MONSTER1:
        return _dwelling[0].monsters;
    case DWELLING_MONSTER2:
    case DWELLING_UPGRADE2:
        return _dwelling[1].monsters;
    case DWELLING_MONSTER3:
    case DWELLING_UPGRADE3:
        return _dwelling[2].monsters;
    case DWELLING_MONSTER4:
    case DWELLING_UPGRADE4:
        return _dwelling[3].monsters;
    case DWELLING_MONSTER5:
    case DWELLING_UPGRADE5:
        return _dwelling[4].monsters;
    case DWELLING_MONSTER6:
    case DWELLING_UPGRADE6:
    case DWELLING_UPGRADE7:
        return _dwelling[5].monsters;
...
OStreamBase & operator<<( OStreamBase & stream, const Castle & castle )
{
    ...
    for ( const Castle::Dwelling & _dwelling : castle._dwelling ) {
        stream << _dwelling.monsters << _dwelling.growth;
    }
...
IStreamBase & operator>>( IStreamBase & stream, Castle & castle )
{
    ...
    if ( const uint32_t size = stream.get32(); castle._dwelling.size() != size ) {
        ...
        for ( Castle::Dwelling & _dwelling : castle._dwelling ) {
            _dwelling = Castle::Dwelling();
        }
    }
    else {
        for ( Castle::Dwelling & _dwelling : castle._dwelling ) {
...
std::string Castle::GetDescriptionBuilding( const uint32_t buildingType ) const
{
    ...
    switch ( buildingType ) {
    case BUILD_CASTLE: {
        StringReplace( res, "%{count}", ProfitConditions::GOLD_BY_CASTLE );
    ...
    case BUILD_SPEC:
        if ( GetRace() == Race::WRLK )
            StringReplace( res, "%{count}", ProfitConditions::GOLD_BY_DANGEON );
        break;
    case BUILD_STATUE:
        StringReplace( res, "%{count}", ProfitConditions::GOLD_BY_STATUE );
        break;
    case BUILD_TAVERN:
        StringReplace( res, "%{count}", ProfitConditions::GOLD_BY_TAVERN );
        break;
    case BUILD_THIEVESGUILD:
        StringReplace( res, "%{count}", ProfitConditions::GOLD_BY_GUILD );
        break;
