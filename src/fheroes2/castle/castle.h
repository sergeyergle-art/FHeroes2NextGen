...
public:
    ...
    {
        return GetColor() == PlayerColor::NONE;
    }
    ...
    bool hasUpperArcherTower() const
    {
        return isBuild( BUILD_LEFTTURRET );
    }

    bool hasLowerArcherTower() const
    {
        return isBuild( BUILD_RIGHTTURRET );
    }
    ...
    uint32_t GetMonstersGrowSpeed( uint32_t dwellingId, bool weekBonus ) const;
    uint32_t GetMonstersPerMonth(uint32_t dwellingId) const;

    // return [count, days];
    std::pair<uint32_t, uint32_t> GetPotentialMonstersGrowth( uint32_t dwellingId ) const;
...
private:
    ...
    Dwelling * _getDwelling( const uint32_t buildingType );
    const Dwelling * _getDwelling( const uint32_t buildingType ) const;
    ...
    std::array<Dwelling, maxNumOfDwellings> _dwelling{ 0 };
    Army _army{ &_captain };
};
