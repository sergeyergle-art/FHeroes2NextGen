...
uint32_t WorldPathfinder::getMovementPenalty( const int from, const int to, const int direction ) const
{
    ...
    uint32_t penalty = Maps::Ground::GetPenalty( fromTile, _pathfindingSkill, fromTile.isRoad() && toTile.isRoad(), Direction::isDiagonal( direction ) );
    ...
    if ( getMaxMovePoints( fromTile.isWater() ) > 0 ) {
        ...
        const uint32_t fromTilePenalty = Maps::Ground::GetPenalty( fromTile, _pathfindingSkill, fromTile.isRoad(), false );
...
void WorldPathfinder::reset()
{
    ...
    _pathfindingSkill = Skill::MaxLevel;
}
...
uint32_t AIWorldPathfinder::getDistance( const int start, const int targetIndex, const PlayerColor color, const double armyStrength,
                                         const uint8_t skill /* = Skill::MaxLevel */ )
{
