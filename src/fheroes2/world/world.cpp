...
void World::NewDay()
{
    ...
    if ( BeginWeek() ) {
        ...
        if (_week > 1) {
            _weekInfo.ChangeWeek(BeginMonth(), _seed);
        }
    }
...
void World::NewMonth()
{
    if ( _month > 1 && _weekInfo.IsMonstersWeek() ) {
        _monthOfMonstersAction( Monster( _weekInfo.GetMonster() ) );
    }
}
...
OStreamBase & operator<<( OStreamBase & stream, const World & w )
{
    return stream << w.width << w.height << w.vec_tiles << w.vec_heroes << w.vec_castles << w.vec_kingdoms << w._customRumors << w.vec_eventsday << w.map_captureobj
                  << w._ultimateArtifact << w._day << w._week << w._month << w.heroIdAsWinCondition << w.heroIdAsLossCondition << w.map_objects << w._seed << w._weekInfo;
}
...
IStreamBase & operator>>( IStreamBase & stream, World & w )
{
    ...
    stream >> w.map_objects >> w._seed >> w._weekInfo;
