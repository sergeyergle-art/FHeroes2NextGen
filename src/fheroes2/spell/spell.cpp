...
namespace
{
    ...
    const std::array<SpellStats, Spell::SPELL_COUNT> spells = { {
        ...
        { gettext_noop( "Fireball" ), 12, 0, 0, 8, 30, gettext_noop( "Causes a giant fireball to strike the selected area, damaging all nearby creatures." ) },
        { gettext_noop( "Fireblast" ), 24, 0, 0, 9, 40, gettext_noop( "An improved version of fireball, fireblast affects two hexes around the center point of the spell, rather than one." ) },
        { gettext_noop( "Lightning Bolt" ), 7, 0, 0, 4, 35, gettext_noop( "Causes a bolt of electrical energy to strike the selected creature." ) },
        { gettext_noop( "Chain Lightning" ), 25, 0, 0, 5, 50, gettext_noop( "Causes a bolt of electrical energy to strike a selected creature, then strike the nearest creature with half damage, then strike the NEXT nearest creature with half again damage, and so on, until it becomes too weak to be harmful. Warning: This spell can hit your own creatures!" ) },
        { gettext_noop( "Teleport" ), 9, 0, 0, 10, 0, gettext_noop( "Teleports the creature you select to any open position on the battlefield." ) },
        { gettext_noop( "Cure" ), 5, 0, 0, 6, 10, gettext_noop( "Removes all negative spells cast upon one of your units, and restores up to %{count} HP per level of spell power." ) },
        { gettext_noop( "Mass Cure" ), 20, 0, 0, 60, 10, gettext_noop( "Removes all negative spells cast upon your forces, and restores up to %{count} HP per level of spell power, per creature." ) },
        { gettext_noop( "Resurrect" ), 15, 0, 0, 13, 75, gettext_noop( "Resurrects creatures from a damaged or dead unit until end of combat." ) },
        { gettext_noop( "Resurrect True" ), 20, 0, 0, 12, 100, gettext_noop( "Resurrects creatures from a damaged or dead unit permanently." ) },
        { gettext_noop( "Haste" ), 3, 0, 0, 14, 3, gettext_noop( "Increases the speed of any creature by %{count}." ) },
        { gettext_noop( "Mass Haste" ), 12, 0, 0, 61, 3, gettext_noop( "Increases the speed of all of your creatures by %{count}." ) },
        { gettext_noop( "spell|Slow" ), 3, 0, 0, 1, 0, gettext_noop( "Slows target to half movement rate." ) },
        { gettext_noop( "Mass Slow" ), 12, 0, 0, 62, 0, gettext_noop( "Slows all enemies to half movement rate." ) },
        { gettext_noop( "spell|Blind" ), 6, 0, 0, 21, 50, gettext_noop( "Clouds the affected creatures' eyes, preventing them from moving and reduces the damage when retaliating by %{count} percent." ) },
        ...
        { gettext_noop( "Holy Word" ), 12, 0, 0, 22, 15, gettext_noop( "Damages all undead in the battle." ) },
        { gettext_noop( "Holy Shout" ), 24, 0, 0, 23, 40, gettext_noop( "Damages all undead in the battle. This is an improved version of Holy Word." ) },
        ...
        { gettext_noop( "Mass Dispel" ), 20, 0, 0, 73, 0, gettext_noop( "Removes all magic spells from all creatures." ) },
        { gettext_noop( "Magic Arrow" ), 3, 0, 0, 38, 15, gettext_noop( "Causes a magic arrow to strike the selected target." ) },
        { gettext_noop( "Berserker" ), 12, 0, 0, 19, 0, gettext_noop( "Causes a creature to attack its nearest neighbor." ) },
        { gettext_noop( "Armageddon" ), 40, 0, 0, 16, 100, gettext_noop( "Holy terror strikes the battlefield, causing severe damage to all creatures." ) },
        { gettext_noop( "Elemental Storm" ), 30, 0, 0, 11, 75, gettext_noop( "Magical elements pour down on the battlefield, damaging all creatures." ) },
        { gettext_noop( "Meteor Shower" ), 20, 0, 0, 24, 50, gettext_noop( "A rain of rocks strikes an area of the battlefield, damaging all nearby creatures." ) },
        { gettext_noop( "Paralyze" ), 9, 0, 0, 20, 0, gettext_noop( "The targeted creatures are paralyzed, unable to move or retaliate." ) },
        { gettext_noop( "Hypnotize" ), 15, 0, 0, 37, 75, gettext_noop( "Brings a single enemy unit under your control if its hits are less than %{count} times the caster's spell power." ) },
        { gettext_noop( "Cold Ray" ), 6, 0, 0, 36, 30, gettext_noop( "Drains body heat from a single enemy unit." ) },
        { gettext_noop( "Cold Ring" ), 10, 0, 0, 35, 25, gettext_noop( "Drains body heat from all units surrounding the center point, but not including the center point." ) },
        { gettext_noop( "Disrupting Ray" ), 7, 0, 0, 34, 3, gettext_noop( "Reduces the defense rating of an enemy unit by three." ) },
        { gettext_noop( "Death Ripple" ), 8, 0, 0, 29, 15, gettext_noop( "Damages all living (non-undead) units in the battle." ) },
        { gettext_noop( "Death Wave" ), 20, 0, 0, 28, 25, gettext_noop( "Damages all living (non-undead) units in the battle. This spell is an improved version of Death Ripple." ) },
        ...
        { gettext_noop( "Animate Dead" ), 15, 0, 0, 25, 75, gettext_noop( "Resurrects creatures from a damaged or dead undead unit permanently." ) },
        { gettext_noop( "Mirror Image" ), 25, 0, 0, 26, 0, gettext_noop( "Creates an illusionary unit that duplicates one of your existing units. This illusionary unit does the same damages as the original, but will vanish if it takes any damage." ) },
        { gettext_noop( "Shield" ), 3, 0, 0, 15, 2, gettext_noop( "Halves damage received from ranged attacks for a single unit. Does not affect damage received from Turrets or Ballistae." ) },
        { gettext_noop( "Mass Shield" ), 12, 0, 0, 65, 0, gettext_noop( "Halves damage received from ranged attacks for all of your units. Does not affect damage received from Turrets or Ballistae." ) },
        ...
        { gettext_noop( "Summon Boat" ), 5, 0, 0, 46, 0, gettext_noop( "Summons the nearest unoccupied, friendly boat to an adjacent shore location. A friendly boat is one which you just built or were the most recent player to occupy." ) },
        { gettext_noop( "Dimension Door" ), 20, 225, 69, 47, 0, gettext_noop( "Allows the caster to magically transport to a nearby location." ) },
        { gettext_noop( "Town Gate" ), 25, 225, 69, 48, 0, gettext_noop( "Returns the caster to the nearest town or castle currently owned. This spell cannot be cast if the hero is already in a town or a castle." ) },
        { gettext_noop( "Town Portal" ), 30, 225, 69, 49, 0, gettext_noop( "Returns the hero to the town or castle of choice, provided it is controlled by you." ) },
        ...
        { gettext_noop( "Haunt" ), 8, 0, 0, 51, 4, gettext_noop( "Haunts a mine you control with Ghosts. This mine stops producing resources. (If I can't keep it, nobody will!)" ) },
        ...
        { gettext_noop( "Petrification" ), 10, 0, 0, 66, 50, gettext_noop( "Turns the affected creature into stone. A petrified creature receives half damage from a direct attack." ) },
    } };
}
...
double Spell::getStrategicValue( const double armyStrength, const uint32_t currentSpellPoints, const int spellPower ) const
{
    ...
    if ( isResurrect() || isMassActions() || id == Spell::BLIND || id == Spell::PARALYZE || id == Spell::PETRIFY ) {
