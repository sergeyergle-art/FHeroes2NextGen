...
int Monster::GetMonAttack() const
{
    return fheroes2::getMonBattleStats( id ).attack;
}

int Monster::GetMonDefense() const
{
    return fheroes2::getMonBattleStats( id ).defense;
}
...
{
    return !isAbilityPresent( fheroes2::MonAbil::EXTRA_LUCK ) ? Luck::NORMAL : 2;
}

int Monster::GetRace() const
{
    return fheroes2::getMonGeneralStats( id ).race;
}

uint32_t Monster::GetShots() const
{
    return fheroes2::getMonBattleStats( id ).shots;
}

double Monster::GetMonsterStrength() const
{
    if ( isValid() ) {
        int attack = GetMonAttack() - 1;
        if ( attack < 0 ) {
            attack = 0;
        }

        int defense = GetMonDefense() - 1;
        if ( defense < 0 ) {
            defense = 0;
        }

        double ad = 1.0 + (attack + defense) * 0.05;
        return ad * fheroes2::getMonBattleStats( id ).monsterBaseStrength / fheroes2::MonsterBattleStats::STRENGTH_FACTOR;
    }
    return 0;
}

double Monster::GetMonsterStrengthWithBonus( int aBonus, int dBonus ) const
{
    if ( isValid() ) {
        auto& stats = fheroes2::getMonBattleStats( id );

        int attack = stats.attack + aBonus - 1;
        if ( attack < 0 ) {
            attack = 0;
        }

        int defense = stats.defense + dBonus - 1;
        if ( defense < 0 ) {
            defense = 0;
        }

        double ad = 1.0 + (attack + defense) * 0.05;
        return ad * stats.monsterBaseStrength / fheroes2::MonsterBattleStats::STRENGTH_FACTOR;
    }
    return 0;
}
...
bool Monster::isAbilityPresent( const fheroes2::MonAbil abilityType ) const
{
    return fheroes2::getMonBattleStats( id ).hasAbil( abilityType );
}
...
{
    return _( fheroes2::getMonName( id ).untranslated );
}

const char * Monster::GetMultiName() const
{
    return _( fheroes2::getMonName( id ).untranslatedPlural );
}

const char * Monster::GetPluralName( uint32_t count ) const
{
    const fheroes2::MonsterName & names = fheroes2::getMonName( id );
    return count == 1 ? _( names.untranslated ) : _( names.untranslatedPlural );
}
...
uint32_t Monster::GetCountFromHitPoints( const Monster & mons, const uint32_t hp )
{
    ...
    assert( singleMonsterHP );

    return ( hp + singleMonsterHP - 1 ) / singleMonsterHP;
}
