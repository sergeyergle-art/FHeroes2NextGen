...
void Battle::Arena::BattleProcess( Unit & attacker, Unit & defender, int32_t tgt /* = -1 */, int dir /* = -1 */ )
{
    ...
    if ( spell.isValid() ) {
        ...
        if ( !spellTargets.empty() ) {
            ...
            _usedSpellsByAttacker.Append( spell );
            _usedSpellsByDefender.Append( spell );
        }
    }
...
void Battle::Arena::ApplyActionSpellCast( Command & cmd )
{
    ...
    auto& currentForce = GetCurrentForce();
    HeroBase * commander = currentForce.GetCommander();
    ...
    if ( IsAttacker( currentForce ) ) {
        _usedSpellsByAttacker.Append( spell );
    }
    else if ( IsDefender( currentForce ) ) {
        _usedSpellsByDefender.Append( spell );
    }
    else {
        assert( 0 );
...
Battle::TargetsInfo Battle::Arena::GetTargetsForDamage( const Unit & attacker, Unit & defender, const int32_t dst, const Battle::CellDirection dir ) const
{
    ...
    {
        if ( fheroes2::getMonBattleStats( attacker.GetID() ).hasAbil( fheroes2::MonAbil::ENEMY_HALVING ) ) {
            const uint32_t halvingDamage = ( defender.GetCount() / 2 + defender.GetCount() % 2 ) * defender.Monster::GetHitPoints();
            if ( halvingDamage > res.damage && Rand::GetWithGen( 1, 100, _randomGenerator ) <= 10 ) {
    ...
    if ( attacker.isDoubleCellAttack() ) {
        ...
        if ( enemy && (enemy->GetColor() != attacker.GetColor()) && consideredTargets.insert( enemy ).second ) {
            res.defender = enemy;
            res.damage = ( attacker.GetDamage( *enemy, _randomGenerator ) * 3 + 2 ) / 4;
            if (res.damage < 1)
                res.damage = 1;
    ...
    // lich cloud damage
    else if ( attacker.isAbilityPresent( fheroes2::MonAbil::AREA_SHOT ) && !Unit::isHandFighting( attacker, defender ) ) {
...
{
    for ( int shots = cmd.GetNextValue(); shots > 0; --shots ) {
        auto target = static_cast<CastleFortress::TargetId>( cmd.GetNextValue() );
        auto hp = cmd.GetNextValue();

        bool isHit = false;

        if ( CastleFortress::IsWall( target ) ) {
            if ( hp < board[CastleFortress::GetTargetCellIdx( target )].GetObject() ) {
                isHit = true;
            }
        }
        else {
        ...
        if ( _interface ) {
            _interface->RedrawActionCatapultPart1( target, isHit );
        }
        ...
        applyDamageToCastleDefenseStructure( target, hp );
...
void Battle::Arena::_applyActionSpellEarthquake()
{
    ...
    CastleFortress::HitInfo hits[CastleFortress::MAX_EARTHQUAKE_HITS];
    int countHits = _fortress.AttackEarthquake( commander->GetPower(), hits, _randomGenerator );
    if ( countHits <= 0 ) {
        assert( 0 );
        return;
    }

    bool isBridgeDestroyed = false;

    CastleFortress::TargetId targets[CastleFortress::MAX_EARTHQUAKE_HITS];
    for ( int i = 0; i < countHits; ++i ) {
        targets[i] = hits[i].stuctId;

        if ( hits[i].stuctId == CastleFortress::GATE && hits[i].hp == 0 ) {
    ...
    if ( _interface ) {
        ...
        _interface->redrawActionEarthquakeSpellPart1( *commander, targets, countHits );
    }

    for ( int i = 0; i < countHits; ++i ) {
        applyDamageToCastleDefenseStructure( hits[i].stuctId, hits[i].hp );
    }

    if ( _interface ) {
        // Render a second part of blast animation after targets are damaged or even destroyed.
        _interface->redrawActionEarthquakeSpellPart2( isBridgeDestroyed, targets, countHits );
    }
}
