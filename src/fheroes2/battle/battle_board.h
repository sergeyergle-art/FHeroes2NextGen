...
namespace Battle
{
    ...
    public:
        ...
        static constexpr int widthInCells{ 15 /*11*/ };
        // Height of the battlefield, measured in cells
        static constexpr int heightInCells{ 11 /*9*/ };
        // Total number of cells on the battlefield
        static constexpr int sizeInCells{ widthInCells * heightInCells };

        static constexpr int pxOffsetX{ -8 };
        static constexpr int pxOffsetY{ -9 };

        static constexpr int centerRow{ heightInCells / 2 };
        static constexpr int catapultRow{ 8 };

        static constexpr int CellIdx(const int x, const int y)
        {
            return x + y * widthInCells;
        }

        static constexpr int CellIdxLeft(const int y)
        {
            return y * widthInCells;
        }

        static constexpr int CellIdxRight(const int y)
        {
            return y * widthInCells + widthInCells - 1;
        }

        static int FirstRowCellIdx( int row )
        {
            return row * widthInCells;
        }

        static int LastRowCellIdx( int row )
        {
            return row * widthInCells + widthInCells - 1;
        }

        static int FirstRowCellIdx( int row, bool opposite )
        {
            return !opposite ? FirstRowCellIdx( row ) : LastRowCellIdx( row );
        }

        static constexpr int firstSpreadTroopsRow = 1; // 0 for area 11 x 9
        static constexpr int firstCompactTroopsRow = 3; // 2 for area 11 x 9

        static int GetFirstTroopsRow( bool isSpread )
        {
            return isSpread ? Board::firstSpreadTroopsRow : Board::firstCompactTroopsRow;
        }

        static bool IsBridgeCell( int idx );
        ...
        Cell& AtCell( int x, int y );
        const Cell& AtCell(int x, int y) const;
    ...
    constexpr int MOAT_IDX_0 = Board::CellIdxRight( 0 ) - 3;
    constexpr int MOAT_IDX_1 = Board::CellIdxRight( 1 ) - 3;
    constexpr int MOAT_IDX_2 = Board::CellIdxRight( 2 ) - 4;
    constexpr int MOAT_IDX_3 = Board::CellIdxRight( 3 ) - 4;
    constexpr int MOAT_IDX_4 = Board::CellIdxRight( 4 ) - 5;
    constexpr int MOAT_IDX_5 = Board::CellIdxRight( 5 ) - 5;
    constexpr int MOAT_IDX_6 = Board::CellIdxRight( 6 ) - 5;
    constexpr int MOAT_IDX_7 = Board::CellIdxRight( 7 ) - 4;
    constexpr int MOAT_IDX_8 = Board::CellIdxRight( 8 ) - 4;
    constexpr int MOAT_IDX_9 = Board::CellIdxRight( 9 ) - 3;
    constexpr int MOAT_IDX_10 = Board::CellIdxRight( 10 ) - 3;

    constexpr int BRIDGE_IDX = Board::CellIdxRight( Board::centerRow ) - 5;
}
