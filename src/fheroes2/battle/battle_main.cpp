...
Battle::Result Battle::Loader( Army & attackingArmy, Army & defendingArmy, const int32_t tileIndex )
{
    ...
    while ( true ) {
        ...
        arena.UpdateSpellsInBookByBattle();
        ...
        if ( isHumanBattle
            ...
            arena.ResetSpellsInBookByBattle();

            continue;
        }

        if ( !showBattle && conf.BattleAutoResolve() && isHumanBattle ) {
            arena.ShowAllAddedSpellsInBookByBattle();
        }
