...
namespace
{
    ...
    constexpr fheroes2::Point bridgeDestroySmokeOffset( 0, 65 );
    ...
    constexpr int32_t battleLogElementWidth{ Battle::Interface::WND_WIDTH - 32 - 16 };
    ...
    {
        return isLeftOpponent ?
            ( ( cellindex % Battle::Board::widthInCells ) < cellindex / Battle::Board::widthInCells - 2 )
                              :
            ( ( (Battle::Board::widthInCells - 1) - ( cellindex % Battle::Board::widthInCells ) ) < cellindex / Battle::Board::widthInCells - 2 );
    }
...
namespace Battle
{
    ...
    public:
        ...
        void SetPosition( int32_t px, int32_t py )
        {
            ...
            _border.SetPosition( px, py - totalElementHeight - 32, Battle::Interface::WND_WIDTH - 32, totalElementHeight );
    ...
    , _isFlippedHorizontally( isReflect )
{
    ...
    if ( _isFlippedHorizontally ) {
        _area.x = _offset.x + Battle::Interface::WND_WIDTH - RIGHT_HERO_X_OFFSET - ( sprite.x() + _area.width );
...
void Battle::OpponentSprite::Redraw( fheroes2::Image & dst ) const
{
    ...
    if ( _isFlippedHorizontally ) {
        fheroes2::Blit( hero, dst, offset.x + Battle::Interface::WND_WIDTH - RIGHT_HERO_X_OFFSET - ( hero.x() + hero.width() ),
...
Battle::Status::Status()
    : _upperBackground( Assets::getImage( ICN::SMALLBAR /*TEXTBAR*/, 0 ) )
    //, _lowerBackground( Assets::getImage( ICN::TEXTBAR, 9 ) )
{
    width = _upperBackground.width() + 107;
    height = _upperBackground.height(); // + _lowerBackground.height();
    _background.resize(width, height);
    fheroes2::Resize(_upperBackground, _background);
}
...
void Battle::Status::redraw( fheroes2::Image & output ) const
{
    ...
    fheroes2::Copy( _background, 0, 0, output, x - 2, y, width, height );
    ...
    : arena( battleArena )
{
    ...
    _interfacePosition = { ( display.width() - WND_WIDTH ) / 2, ( display.height() - WND_HEIGHT ) / 2, _surfaceInnerArea.width, _surfaceInnerArea.height };
    border.SetPosition( _interfacePosition.x - fheroes2::borderWidthPx, _interfacePosition.y - fheroes2::borderWidthPx, WND_WIDTH, WND_HEIGHT );
    ...
    _brightLandType = (
        groundType == Maps::Ground::SNOW ||
        groundType == Maps::Ground::DESERT ||
        groundType == Maps::Ground::WASTELAND ||
    ...
    if ( Arena::GetCastle() ) {
        _ballistaTowerRect = fheroes2::Rect{ _interfacePosition.x + WND_WIDTH - 70, _interfacePosition.y + 145, 70, 160 };
    }
    ...
    status.setPosition( area.x + settingsRect.width, area.y + area.height - status.height );
    ...
    _mainSurface.resize( area.width, area.height );
    _battleGround.resize( area.width, area.height );
...
{
    if ( !_background ) {
        _background = std::make_unique<fheroes2::StandardWindow>( WND_WIDTH, WND_HEIGHT, false );
    }
...
void Battle::Interface::RedrawArmies()
{
    ...
    static const std::array<int32_t, Board::heightInCells> wallCellIds{
        CastleFortress::POS_UPPER_BOUND_1,
        CastleFortress::POS_UPPER_WALL_1,
        CastleFortress::POS_UPPER_TOWER,
        CastleFortress::POS_UPPER_WALL_2,
        CastleFortress::POS_GATE_TOWER_1,
        CastleFortress::POS_GATE,
        CastleFortress::POS_GATE_TOWER_2,
        CastleFortress::POS_LOWER_WALL_1,
        CastleFortress::POS_LOWER_TOWER,
        CastleFortress::POS_LOWER_WALL_2,
        CastleFortress::POS_EMPTY_TOWER
    };
    ...
    for ( int32_t cellRowId = 0; cellRowId < Board::heightInCells; ++cellRowId ) {
        ...
        if ( castle != nullptr ) {
            // Redraw main tower.
            if ( cellRowId == Board::centerRow ) {
                RedrawCastleMainTower( *castle );
            }
            else if ( cellRowId == Board::catapultRow ) {
                RedrawCatapult();
            }
            ...
            for ( int32_t cellColumnId = 0; cellColumnId < Board::widthInCells; ++cellColumnId ) {
                ...
                if ( cellRowId <= Board::centerRow ) {
                    ...
                    isCellBefore = cellId > wallCellId;
                    if ( ( wallCellId == CastleFortress::POS_LOWER_WALL_1 || wallCellId == CastleFortress::POS_LOWER_WALL_2 )
...
void Battle::Interface::RedrawOpponentsFlags()
{
    ...
    if ( _defendingOpponent ) {
        ...
        fheroes2::Blit( flag, _mainSurface,
                        _defendingOpponent->Offset().x + WND_WIDTH - OpponentSprite::RIGHT_HERO_X_OFFSET - ( flag.x() + flag.width() ),
...
fheroes2::Point Battle::Interface::_drawTroopSprite( const Unit & unit, const fheroes2::Sprite & troopSprite )
{
    ...
    if ( _movingUnit == &unit ) {
        ...
        if ( _movingUnit->animation.animationLength() ) {
            ...
            if ( _movingUnit->isAbilityPresent( fheroes2::MonAbil::FLYING ) ) {
...
void Battle::Interface::RedrawCover()
{
    ...
    if ( bridge && ( bridge->isDown() || _bridgeAnimation.animationIsRequired ) ) {
        ...
        fheroes2::Blit( bridgeImage, _mainSurface, bridgeImage.x() + CASTLE_OFFSET_X, bridgeImage.y() + CASTLE_OFFSET_Y );
    }
    ...
    if ( cell && _currentUnit && Settings::Get().BattleShowMouseShadow() ) {
        ...
        else if ( _currentUnit->isAbilityPresent( fheroes2::MonAbil::AREA_SHOT )
...
void Battle::Interface::_redrawBattleGround()
{
    ...
    if ( _battleGroundIcn != ICN::UNKNOWN ) {
        ...
        if (cbkg.width() != _battleGround.width() || cbkg.height() != _battleGround.height())
        {
            fheroes2::Copy( fheroes2::Stretch(cbkg, 0, 0, cbkg.width(), cbkg.height(), _battleGround.width(), _battleGround.height()), _battleGround );
        }
        else
        {
    ...
    if ( arena.GetICNCovr() != ICN::UNKNOWN ) {
        ...
        int x = cover.x() + Board::pxOffsetX + Cell::widthPx * 2;
        int y = cover.y() + Board::pxOffsetY;
        fheroes2::Blit( cover, _battleGround, x, y );
    }
    ...
    if ( castle != nullptr ) {
...

        {
            ...
            fheroes2::Image cstl( castleBackground.width() + Cell::widthPx / 2, castleBackground.height() + BOARD_INC_HEIGHT );
            fheroes2::Resize( castleBackground, cstl );
            fheroes2::Blit( cstl, _battleGround, castleBackground.x() + CASTLE_OFFSET_X, castleBackground.y() );
        }

        // Moat.
        if ( castle->isBuild( BUILD_MOAT ) ) {
            const fheroes2::Sprite & sprite = Assets::getImage( ICN::MOATWHOL, 0 );
            fheroes2::Image moat( sprite.width() + Cell::widthPx / 2, sprite.height() + BOARD_INC_HEIGHT );
            fheroes2::Resize( sprite, moat );
            fheroes2::Blit( moat, _battleGround, sprite.x() + CASTLE_OFFSET_X, sprite.y() );
        }
    }
    ...
    if ( castle != nullptr ) {
        ...
        fheroes2::Image wl = fheroes2::Stretch(sprite2, 0, 0, sprite2.width(), sprite2.height(), sprite2.width() + 22, sprite2.height());
        fheroes2::Blit( wl, _battleGround, sprite2.x() + CASTLE_OFFSET_X, sprite2.y() + CASTLE_OFFSET_Y );
    }
}
...
void Battle::Interface::RedrawCatapult()
{
    const fheroes2::Sprite & sprite = Assets::getImage( ICN::CATAPULT, catapult_frame );
    const Cell& cell = Arena::GetBoard()->at( POS_CATAPULT );
    int x = sprite.x() + cell.GetPos().x - Cell::widthPx * 6 / 5;
    int y = sprite.y() + cell.GetPos().y + Cell::heightPx * 3 / 5;
    fheroes2::Blit( sprite, _mainSurface, x, y );
}
...
void Battle::Interface::RedrawCastle( const Castle & castle, const int32_t cellId )
{
    ...
    if ( CastleFortress::POS_GATE == cellId ) {
        const Bridge * bridge = Arena::GetBridge();
        assert( bridge != nullptr );
        if ( bridge != nullptr && !bridge->isDestroyed() ) {
            const fheroes2::Sprite & sprite = Assets::getImage( castleIcnId, 4 );
            int x = sprite.x() + CASTLE_OFFSET_X;
            int y = sprite.y() + CASTLE_OFFSET_Y;
            fheroes2::Blit( sprite, _mainSurface, x, y );
        }
    }
    else if (
        CastleFortress::POS_UPPER_WALL_1 == cellId ||
        CastleFortress::POS_UPPER_WALL_2 == cellId ||
        CastleFortress::POS_LOWER_WALL_1 == cellId ||
        CastleFortress::POS_LOWER_WALL_2 == cellId ) {

        CastleFortress::TargetId tagId = CastleFortress::ALL_TARGETS;

        uint32_t index = 0;

        switch ( cellId ) {
        case CastleFortress::POS_UPPER_WALL_1:
            tagId = CastleFortress::UPPER_WALL_1;
            index = 5;
            break;
        case CastleFortress::POS_UPPER_WALL_2:
            tagId = CastleFortress::UPPER_WALL_2;
            index = 6;
            break;
        case CastleFortress::POS_LOWER_WALL_1:
            tagId = CastleFortress::LOWER_WALL_1;
            index = 7;
            break;
        case CastleFortress::POS_LOWER_WALL_2:
            tagId = CastleFortress::LOWER_WALL_2;
            index = 7; //8;
            break;
        default:
            break;
        ...
        int x = sprite.x() + CASTLE_OFFSET_X;
        int y = sprite.y() + CASTLE_OFFSET_Y;
        if (cellId == CastleFortress::POS_LOWER_WALL_2) {
            x += 46;
            y += 83;
        }
        fheroes2::Blit( sprite, _mainSurface, x, y );
    }
    else if ( CastleFortress::POS_UPPER_TOWER == cellId ) {
        uint32_t index = ( Board::GetCell( cellId )->GetObject() != 1 )
            ? ( castle.hasUpperArcherTower() ? 18 : 17 ) : 19 ;

        const fheroes2::Sprite & towerSprite = Assets::getImage( castleIcnId, index );
        int x = 443 + towerSprite.x() + CASTLE_OFFSET_X;
        int y = 153 + towerSprite.y() + CASTLE_OFFSET_Y;
        fheroes2::Blit( towerSprite, _mainSurface, x, y );
    }
    else if ( CastleFortress::POS_LOWER_TOWER == cellId ) {
        uint32_t index = ( Board::GetCell( cellId )->GetObject() != 1 )
            ? ( castle.hasLowerArcherTower() ? 18 : 17 ) : 19 ;

        const fheroes2::Sprite & towerSprite = Assets::getImage( castleIcnId, index );
        int x = 443 + towerSprite.x() + CASTLE_OFFSET_X;
        int y = 405 + towerSprite.y() + CASTLE_OFFSET_Y;
        fheroes2::Blit( towerSprite, _mainSurface, x, y );
    }
    else if ( CastleFortress::POS_GATE_TOWER_1 == cellId ) {
        uint32_t index = ( Board::GetCell( cellId )->GetObject() != 1 )
            ? ( castle.hasUpperArcherTower() ? 18 : 17 ) : 19 ;

        const fheroes2::Sprite & towerSprite = Assets::getImage( castleIcnId, index );
        int x = 399 + towerSprite.x() + CASTLE_OFFSET_X;
        int y = 237 + towerSprite.y() + CASTLE_OFFSET_Y;
        fheroes2::Blit( towerSprite, _mainSurface, x, y );
    }
    else if ( CastleFortress::POS_GATE_TOWER_2 == cellId ) {
        uint32_t index = ( Board::GetCell( cellId )->GetObject() != 1 )
            ? ( castle.hasLowerArcherTower() ? 18 : 17 ) : 19 ;

        const fheroes2::Sprite & towerSprite = Assets::getImage( castleIcnId, index );
        int x = 399 + towerSprite.x() + CASTLE_OFFSET_X;
        int y = 321 + towerSprite.y() + CASTLE_OFFSET_Y;
        fheroes2::Blit( towerSprite, _mainSurface, x, y );
    }
    else if ( CastleFortress::POS_EMPTY_TOWER == cellId ) {
        const Cell& cell = Arena::GetBoard()->at(cellId);

        const fheroes2::Sprite & log = Assets::getImage( ICN::COBJ0011, 0 );
        int x = cell.GetPos().x + log.x() + 72;
        int y = cell.GetPos().y + log.y() + 48;
        fheroes2::Blit( log, _mainSurface, x, y );

        uint32_t index = ( Board::GetCell( cellId )->GetObject() != 1 )
            ? ( ( castle.hasUpperArcherTower() && castle.hasLowerArcherTower() ) ? 18 : 17 ) : 19 ;

        const fheroes2::Sprite & twr = Assets::getImage( castleIcnId, index );
        x = cell.GetPos().x + 5;
        y = cell.GetPos().y - twr.height() + 50;
        fheroes2::Blit( twr, _mainSurface, x, y );

        if ( index != 19 ) {
            index = castle.isFortificationBuilt() ? 31 : 8;
        }
        else {
            index = castle.isFortificationBuilt() ? 39 : 16;
        }

        const fheroes2::Sprite & wl = Assets::getImage( castleIcnId, index );
        x = cell.GetPos().x + 28;
        y = cell.GetPos().y - 10;
        fheroes2::Blit( wl, _mainSurface, x, y );
    }
}
...
void Battle::Interface::RedrawCastleMainTower( const Castle & castle )
{
    ...
    int x = WND_WIDTH - sprite.width();
    int y = sprite.y() + CASTLE_OFFSET_Y - Cell::heightPx * 2 / 3;
    fheroes2::Blit( sprite, _mainSurface, x, y );
}
...
void Battle::Interface::ShowSpellsList( const std::vector<Spell>& spells, const HeroBase* hero )
{
    if ( !spells.empty() ) {
...
void Battle::Interface::RedrawActionAttackPart2( Unit & attacker, const Unit & defender, const TargetsInfo & targets, const uint32_t resurrects )
{
    ...
    if ( !isMirror && !targets.empty() ) {
        ...
        if ( resurrects != 0 ) {
            ...
            else if ( attacker.isAbilityPresent( fheroes2::MonAbil::HP_DRAIN ) ) {
...
void Battle::Interface::RedrawActionMove( Unit & unit, const Indexes & path )
{
    ...
    const bool canFly = unit.isAbilityPresent( fheroes2::MonAbil::FLYING );
...
void Battle::Interface::RedrawActionCatapultPart1( const CastleFortress::TargetId catapultTarget, const bool isHit )
{
    ...
    fheroes2::Point pt2 = CastleFortress::GetTargetPosition( catapultTarget );
    pt1.x += 10;
    pt1.y += 40;
    pt2.x += Cell::widthPx * 4 - Cell::widthPx / 2;
    pt2.y += Cell::heightPx;
    ...
    int32_t boulderArcHeight = static_cast<int32_t>( std::lround( 0.55 * getDistance( pt1, pt2 ) ) );
    ...
    const bool isBridgeDestroyed = isHit && ( catapultTarget == CastleFortress::GATE );
...
void Battle::Interface::RedrawActionCatapultPart2( const CastleFortress::TargetId catapultTarget )
{
    // Finish the smoke cloud animation after the building's state has changed after the hit and it is drawn as demolished.

    fheroes2::Point pt1 = CastleFortress::GetTargetPosition( catapultTarget ) + GetArea().getPosition();
    pt1.x += Cell::widthPx * 4 - Cell::widthPx / 2;
    pt1.y += Cell::heightPx;
    ...
    const bool isBridgeDestroyed = ( catapultTarget == CastleFortress::GATE );
    // If the bridge is destroyed - prepare parameters for the second smoke cloud.
    if ( isBridgeDestroyed ) {
...
void Battle::Interface::redrawActionEarthquakeSpellPart1( const HeroBase & caster, const CastleFortress::TargetId targets[], int countTargets )
{
    Cursor::Get().SetThemes( Cursor::WAR_POINTER );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( allDelays ) ) && frame < maxFrame ) {
        ...
        if ( isNeedSpellAnimationUpdate || _needRedraw ) {
            ...
            for ( int i = 0; i < countTargets; ++i ) {
                const fheroes2::Point pt2 = CastleFortress::GetTargetPosition( targets[i] );

                if ( targets[i] == CastleFortress::GATE && frame >= bridgeDestroySmokeDelay ) {
                    // When a bridge is demolished, there is one additional smoke explosion at the point where the bridge falls.
                    const fheroes2::Sprite & sprite = Assets::getImage( icn, frame - bridgeDestroySmokeDelay );
                    int x = pt2.x + bridgeDestroySmokeOffset.x + sprite.x() + CASTLE_OFFSET_X;
                    int y = pt2.y + bridgeDestroySmokeOffset.y + sprite.y() + CASTLE_OFFSET_Y;

                    fheroes2::Blit( sprite, _mainSurface, x, y );
                }

                const fheroes2::Sprite & spriteCloud = Assets::getImage( icn, frame );
                int x = pt2.x + spriteCloud.x() + CASTLE_OFFSET_X + 20;
                int y = pt2.y + spriteCloud.y() + CASTLE_OFFSET_Y;

                fheroes2::Blit( spriteCloud, _mainSurface, x, y );
            }
...
void Battle::Interface::redrawActionEarthquakeSpellPart2( bool isBridgeDestroyed, const CastleFortress::TargetId targets[], int countTargets )
{
    Cursor::Get().SetThemes( Cursor::WAR_POINTER );
    ...
    while ( le.HandleEvents( Game::isDelayNeeded( allDelays ) ) && frame < maxFrame ) {
        ...
        if ( isNeedSpellAnimationUpdate || _needRedraw ) {
            ...
            for ( int i = 0; i < countTargets; ++i ) {
                ...
                if ( targets[i] == CastleFortress::GATE ) {
                    // When a bridge is demolished, there is one additional smoke explosion at the point where the bridge falls.
