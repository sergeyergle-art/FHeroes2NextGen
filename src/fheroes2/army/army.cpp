...
void Army::setFromTile( const Maps::Tile & tile )
{
    ...
    case MP2::OBJ_PYRAMID:
        at( 0 )->Set( Monster::VAMPIRE_LORD, 15 );
        at( 1 )->Set( Monster::ROYAL_MUMMY, 20 );
        at( 2 )->Set( Monster::POWER_LICH, 5 );
        at( 3 )->Set( Monster::ROYAL_MUMMY, 20 );
        at( 4 )->Set( Monster::VAMPIRE_LORD, 15 );
        break;

    case MP2::OBJ_GRAVEYARD:
        at( 0 )->Set( Monster::MUTANT_ZOMBIE, 30 );
        at( 1 )->Set( Monster::MUTANT_ZOMBIE, 30 );
        at( 2 )->Set( Monster::MUTANT_ZOMBIE, 30 );
        at( 3 )->Set( Monster::MUTANT_ZOMBIE, 30 );
        at( 4 )->Set( Monster::MUTANT_ZOMBIE, 30 );
        break;
    ...
    case MP2::OBJ_DERELICT_SHIP:
        ArrangeForBattle( Monster::SKELETON, 250, tile.GetIndex(), false );
        break;
    ...
    case MP2::OBJ_TROLL_BRIDGE:
        at( 0 )->Set( Monster::TROLL, 5 );
        at( 1 )->Set( Monster::WAR_TROLL, 5 );
        at( 2 )->Set( Monster::TROLL, 5 );
        at( 3 )->Set( Monster::WAR_TROLL, 5 );
        at( 4 )->Set( Monster::TROLL, 5 );
        break;
    ...
    case MP2::OBJ_DRAGON_CITY: {
        ...
        at( 0 )->Set( Monster::GREEN_DRAGON, 3 + monsterCount * 2 );
        at( 1 )->Set( Monster::RED_DRAGON, 2 + monsterCount );
        at( 2 )->Set( Monster::BLACK_DRAGON, 1 + monsterCount );
        at( 3 )->Set( Monster::RED_DRAGON, 2 + monsterCount );
        at( 4 )->Set( Monster::GREEN_DRAGON, 3 + monsterCount * 2 );
        break;
    }

    case MP2::OBJ_DAEMON_CAVE:
        at( 0 )->Set( Monster::EARTH_ELEMENT, 5 );
        at( 1 )->Set( Monster::EARTH_ELEMENT, 5 );
        at( 2 )->Set( Monster::EARTH_ELEMENT, 5 );
        at( 3 )->Set( Monster::EARTH_ELEMENT, 5 );
        at( 4 )->Set( Monster::EARTH_ELEMENT, 5 );
        break;
...
double Army::GetStrength() const
{
    ...
    for ( const Troop * troop : *this ) {
        ...
        if ( troop->isAffectedByMorale() ) {
            double moraleFactor = armyMorale * Morale::CHANCE_IN_PERCENT / 100.0;
            if (armyMorale < 0)
                moraleFactor /= 2;

            strength *= 1 + moraleFactor;
        }

        {
            double luckFactor = armyLuck * Luck::CHANCE_IN_PERCENT / 100.0;
            if (luckFactor < 0)
                luckFactor /= 2;

            strength *= 1 + luckFactor;
        }
...
void Army::Reset( const bool defaultArmy /* = false */ )
{
    ...
    Monster mon1( race, DWELLING_MONSTER1 );
    Monster mon2( race, DWELLING_MONSTER2 );

    int goldCost1 = mon1.GetGoldCost();
    int goldCost2 = mon2.GetGoldCost();

    int maxCount2 = std::min( 1000 / goldCost2, 7 );
    int count2 = maxCount2 > 3 ? static_cast<int>( Rand::Get( 3, maxCount2 ) ) : 3;
    int count1 = ( 1500 - count2 * goldCost2 + goldCost1 / 2 ) / goldCost1;
    if ( count1 >= 0 ) {
        JoinTroop( mon1, count1, false );
    }
    JoinTroop( mon2, count2, false );
}
...
NeutralMonsterJoiningCondition Army::GetJoinSolution( const Heroes & hero, const Maps::Tile & tile, const Troop & troop )
{
    if ( !troop.isValid() ) {
        ...
        return { NeutralMonsterJoiningCondition::Reason::None, 0, 0, nullptr, nullptr };
    }
        ...
        for ( size_t i = 0; i < campaignAwards.size(); ++i ) {
            ...
            while ( true ) {
                if ( troop.GetID() == monster.GetID() ) {
                    if ( isAlliance ) {
                        return { NeutralMonsterJoiningCondition::Reason::Alliance, troop.GetCount(), troop.GetTotalCost().gold,
                        ...
                        return { NeutralMonsterJoiningCondition::Reason::Bane, troop.GetCount(), troop.GetTotalCost().gold, nullptr,
    ...
    if ( canJoin ) {
        if ( Maps::isMonsterOnTileJoinConditionFree( tile ) ) {
            return { NeutralMonsterJoiningCondition::Reason::Free, troop.GetCount(), troop.GetTotalCost().gold, nullptr, nullptr };
        }

        if ( hero.HasSecondarySkill( Skill::Secondary::DIPLOMACY ) ) {
            const uint32_t amountToJoin = ( troop.GetCount() * hero.GetSecondarySkillValue( Skill::Secondary::DIPLOMACY ) + 50 ) / 100;
            ...
            if ( amountToJoin > 0 ) {
                uint32_t goldFactor = 220 - hero.GetLevelSkill( Skill::Secondary::DIPLOMACY ) * 20;
                uint32_t goldCoast = troop.GetCost().gold * amountToJoin;
                goldCoast = (goldCoast * goldFactor + 50 ) / 100;

                if ( hero.GetKingdom().AllowPayment( Funds( Resource::GOLD, goldCoast ) ) ) {
                    return { NeutralMonsterJoiningCondition::Reason::ForMoney, amountToJoin, static_cast<int32_t>(goldCoast), nullptr, nullptr };
                }
            }
        }
    }
    ...
    if ( armyStrengthRatio > 5 && !hero.isControlAI() ) {
        ...
        return { NeutralMonsterJoiningCondition::Reason::RunAway, 0, 0, nullptr, nullptr };
    }
