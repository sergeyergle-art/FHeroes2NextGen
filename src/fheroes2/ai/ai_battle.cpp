...
namespace
{
    const std::array<int32_t, 5> cellsUnderWallsIndexes = {
        Battle::CastleFortress::POS_UPPER_WALL_1 - 1,
        Battle::CastleFortress::POS_UPPER_WALL_2 - 1,
        Battle::CastleFortress::POS_GATE - 1,
        Battle::CastleFortress::POS_LOWER_WALL_1 - 1,
        Battle::CastleFortress::POS_LOWER_WALL_2 - 1
    };
    ...
    double optimalAttackValue( const Battle::Unit & attacker, const Battle::Unit & target, const Battle::Position & attackPos, const double allEnemiesThreat )
    {
        ...
        if ( attacker.isAbilityPresent( fheroes2::MonAbil::SOUL_EATER ) || attacker.isAbilityPresent( fheroes2::MonAbil::HP_DRAIN ) ) {
            ...
            if ( attacker.isAbilityPresent( fheroes2::MonAbil::SOUL_EATER ) ) {
                ...
                assert( attacker.isAbilityPresent( fheroes2::MonAbil::HP_DRAIN ) );

                ressurectPoints = futureKilled * target.Monster::GetHitPoints() / 2;
...
void AI::BattlePlanner::analyzeBattleState( const Battle::Arena & arena, const Battle::Unit & currentUnit )
{
    ...
    for ( const Battle::Unit * unit : enemyForce ) {
        ...
        if ( unit->isArchers() && !unit->isImmovable() ) {
            ...
            if ( unit->isAbilityPresent( fheroes2::MonAbil::AREA_SHOT ) && !unit->isHandFighting() ) {
...
Battle::Actions AI::BattlePlanner::archerDecision( Battle::Arena & arena, const Battle::Unit & currentUnit ) const
{
    ...
    // Archers are blocked and there is nowhere to retreat, they are fighting in melee
        ...
        const bool isAreaShotAbilityPresent = currentUnit.isAbilityPresent( fheroes2::MonAbil::AREA_SHOT );
...
AI::BattleTargetPair AI::BattlePlanner::meleeUnitDefense( Battle::Arena & arena, const Battle::Unit & currentUnit ) const
{
    ...
    if ( currentUnit.GetArmyColor() == _myColor ) {
        ...
        for ( const Battle::Unit * frnd : friendly ) {
            ...
            // next to them.
            if ( !currentUnit.isIgnoringRetaliation() && !frnd->isAbilityPresent( fheroes2::MonAbil::AREA_SHOT ) ) {
                continue;
            }
