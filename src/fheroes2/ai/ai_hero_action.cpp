...
namespace
{
    ...
    void AIToPickupResource( Heroes & hero, const MP2::MapObjectType objectType, int32_t dst_index )
    {
        ...
        if ( objectType != MP2::OBJ_BOTTLE ) {
            hero.GetKingdom().AddFundsResource( getFundsFromTile( tile, hero.GetSecondarySkillValue( Skill::Secondary::EAGLE_EYE ) ) );
        }
    ...
    void AIToDaemonCave( Heroes & hero, int32_t dst_index )
    {
        ...
        if ( doesTileContainValuableItems( tile ) ) {
            ...
            if ( res.isAttackerWin() ) {
                hero.IncreaseExperience( res.getAttackerExperience() );
                // Daemon Cave always gives 10000 Gold after a battle
                hero.GetKingdom().AddFundsResource( Funds( Resource::GOLD, 10000 ) );
            }
            else {
                AIBattleLose( hero, res, true );
            }
