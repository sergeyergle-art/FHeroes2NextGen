...
{
    const double targetDefense = target.GetMonDefense();
                                            ...
                                            const bool forDispel ) const
{
    ...
    case Spell::BLIND: {
        if ( targetIsLast ) {
            ...
            if ( target.isAbilityPresent( fheroes2::MonAbil::UNLIMITED_RETALIATION ) ) {
                // Not so much useful.
                return 0;
            }
...
AI::SpellcastOutcome AI::BattlePlanner::spellEarthquakeValue( const Battle::Arena & arena, const Spell & spell, const Battle::Units & friendly ) const
{
    ...
    const double catapultRatio = Battle::CastleFortress::GetRelativeCatapultPower( *_commander );
    const double targetRatio = arena.GetRelativeEarthquakeDamage( _commander->GetPower() ) / catapultRatio;
    const double meleeRatio = meleeStrength * meleeStrength / _myArmyStrength;

    return { 0, meleeUnits * meleeRatio * targetRatio * enemyShooterRatio * 0.2 };
}
