    ...
    , _race( race )
{
    ...
    if ( _id == DEBUG_HERO ) {
        ...
        for ( const int32_t spellId : Spell::getAllSpellIdsSuitableForSpellBook() ) {
            AppendSpellToBook( Spell( spellId ) );
        }
    }
...
uint32_t Heroes::GetMaxMovePoints( const bool onWater ) const
{
    ...
    if ( onWater ) {
        ...
        result += 400 * world.CountCapturedObject( MP2::OBJ_LIGHTHOUSE, GetColor() );
    }
    else {
        result = 1600;
...
void Heroes::IncreaseExperience( uint32_t amount, const bool autoselect /* = false */ )
{
    uint32_t expFactor = GetSecondarySkillValue(Skill::Secondary::WISDOM);
    if ( expFactor > 0 ) {
        amount += ( amount * expFactor + 50 ) / 100;
    }
...
bool Heroes::CanMove() const
{
    ...
    return _movePoints >= ( Maps::Ground::GetPenalty( tile, GetLevelSkill( Skill::Secondary::PATHFINDING ), tile.isRoad(), false ) );
}
...
uint32_t Heroes::_updateMovementPoints( const uint32_t movePoints, const int skill ) const
{
    ...
    return movePoints + ( skillValue * movePoints + 50 ) / 100;
}
