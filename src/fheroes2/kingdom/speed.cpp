...
namespace Speed
{
    ...
    uint32_t getSlowSpeedFromSpell( const uint32_t currentSpeed )
    {
        ...
        return spellExtraValue ? ((currentSpeed > spellExtraValue) ? (currentSpeed - spellExtraValue) : 1) : getSlowerSpeed( currentSpeed );
    }
    ...
    uint32_t getSlowPriorityFromSpell( const uint32_t currentPriority )
    {
        return currentPriority > 2 ? currentPriority - 2 : 1;
    }

    uint32_t getHastePriorityFromSpell( const uint32_t currentPriority )
    {
