...
Funds Kingdom::GetIncome( int type /* = INCOME_ALL */ ) const
{
    ...
    if ( INCOME_CASTLES & type ) {
        ...
        for ( const Castle * castle : castles ) {
            ...
            totalIncome.AddGold( castle->isCastle() ? ProfitConditions::GOLD_BY_CASTLE : ProfitConditions::GOLD_BY_TENT );

            // statue
            if ( castle->isBuild( BUILD_STATUE ) )
                totalIncome.AddGold( ProfitConditions::GOLD_BY_STATUE );

            // tavern
            if ( castle->isBuild( BUILD_TAVERN ) )
                totalIncome.AddGold( ProfitConditions::GOLD_BY_TAVERN );

            // guild
            if ( castle->isBuild( BUILD_THIEVESGUILD ) )
                totalIncome.AddGold( ProfitConditions::GOLD_BY_GUILD );

            // dungeon for warlock
            if ( castle->isBuild( BUILD_SPEC ) && Race::WRLK == castle->GetRace() ) {
                totalIncome.AddGold( ProfitConditions::GOLD_BY_DANGEON );
                totalIncome.AddSulfur( 1 );
            }
        }
    }
    ...
    if ( INCOME_HERO_SKILLS & type ) {
        ...
        for ( const Heroes * hero : heroes ) {
            assert( hero != nullptr );
            totalIncome.AddGold( hero->GetSecondarySkillValue( Skill::Secondary::ESTATES ) );
        }
    }
