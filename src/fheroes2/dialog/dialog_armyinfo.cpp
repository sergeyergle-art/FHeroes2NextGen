...
namespace
{
    ...
    void DrawMonsterStats( const fheroes2::Point & dst, const Troop & troop, fheroes2::Display & display )
    {
        ...
        if ( fheroes2::getMonBattleStats( troop.GetID() ).shots > 0 ) {
            textPos.y += offsetY;
            text.set( ":", font );
            text.draw( middlePartPosX, textPos.y, display );
        ...
        text.set( _( "Priority" ), font );
        textPos.x = std::max( rightSidePosX - text.width(), minX );
        text.fitToOneRow( leftSideLength );
        text.draw( textPos.x, textPos.y, display );

        text.set( troop.GetPriorityString(), font );
        textPos.x = leftSidePosX;
        text.fitToOneRow( rightSideLength );
        text.draw( textPos.x, textPos.y, display );
        ...
        int morale = troop.GetMorale();
        if ( morale ) {
            std::string moraleTxt = std::to_string( morale );
#ifdef WITH_DEBUG
            moraleTxt += " (";
            moraleTxt += std::to_string( morale * Morale::CHANCE_IN_PERCENT );
            moraleTxt += _( "%)" );
#endif
            text.set( moraleTxt, font );
        }
        else {
            text.set( Morale::String( morale ), font );
        }
        textPos.x = leftSidePosX;
        text.fitToOneRow( rightSideLength );
        text.draw( textPos.x, textPos.y, display );
        ...
        int luck = troop.GetLuck();
        if ( luck ) {
            std::string luckTxt = std::to_string( luck );
#ifdef WITH_DEBUG
            luckTxt += " (";
            luckTxt += std::to_string( luck * Luck::CHANCE_IN_PERCENT );
            luckTxt += _( "%)" );
#endif
            text.set( luckTxt, font );
        }
        else {
            text.set( Luck::String( luck ), font );
        }
        textPos.x = leftSidePosX;
        text.fitToOneRow( rightSideLength );
        text.draw( textPos.x, textPos.y, display );
...
int Dialog::ArmyJoinWithCost( const Troop & troop, const uint32_t join, const uint32_t gold )
{
    ...
    const FrameBox box( 10 + textbox.height( fheroes2::boxAreaWidthPx ) + 10 + sprite.height() + 10, true );
